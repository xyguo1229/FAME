...
class DataLoader(object):
    def __init__(self, dataset, parameter, step='train'):
        ...
        self.tmp_dict = {}
        for relation, quadruples in self.tasks.items():
            # 初始化每个关系名对应的嵌套字典
            time_to_tail = {}

            for quadruple in quadruples:
                head, rel, tail, time = quadruple
                key = (head, time)
                # 将时间元组作为键，尾实体作为值添加到嵌套字典中
                if key not in time_to_tail:
                    time_to_tail[key] = []
                time_to_tail[key].append(tail)

            # 将嵌套字典作为关系的值添加到新字典中
            self.tmp_dict[relation] = time_to_tail
    ...
    def next_one(self):
        ...
        if curr_rel.endswith(' _inv'):
            # 去除 " _inv" 后缀
            curr_rel0 = curr_rel[:-5]
        else:
            curr_rel0 = curr_rel
        ...
        for triple in support_triples:
            e1, rel, e2, time = triple
            while True:
                negative = random.choice(curr_cand)
                if negative != e2 and negative not in self.tmp_dict[curr_rel].get((e1,time),[]):
                    break
            support_negative_triples.append([e1, rel, negative, time])
            # support_negative_triples.append([e1, rel, e1, time])
        ...
        for triple in query_triples:
            ...
            while True:
                ...
                if negative != e2 and negative not in self.tmp_dict[curr_rel].get((e1,time),[]):
                    break
            negative_triples.append([e1, rel, negative, time])
            # negative_triples.append([e1, rel, e1, time])
