...
class Trainer:
    ...
    def do_one_step(self, task, iseval=False, curr_rel=''):
        ...
        if not iseval:
            ...
            p_score, n_score, sup_neg_e1, sup_neg_e2, rel_s, support_t = self.metaR(task, iseval, curr_rel)
            # y = torch.Tensor([1]).to(self.device)
            y = torch.ones_like(p_score).to(self.device)
            loss = self.metaR.loss_func(p_score, n_score, y)

            # e2 = sup_neg_e1 + rel_s + support_t
            # # (h+r,h)x (h,t)x (h+r,t)√
            # pred_head_sim = torch.cosine_similarity(sup_neg_e1, e2, dim=-1).squeeze()
            # head_tail_sim = torch.cosine_similarity(sup_neg_e1, sup_neg_e2, dim=-1).squeeze()
            # pred_tail_sim = torch.cosine_similarity(sup_neg_e2, e2, dim=-1).squeeze()

            e2 = sup_neg_e1[:, :self.num_query, :, :] + rel_s[:, :self.num_query, :, :]
            # (h+r,h)x (h,t)x (h+r,t)√
            pred_head_sim = torch.cosine_similarity(sup_neg_e1[:, :self.num_query, :, :], e2, dim=-1)
            head_tail_sim = torch.cosine_similarity(sup_neg_e1[:, :self.num_query, :, :],
                                                    sup_neg_e2[:, :self.num_query, :, :], dim=-1)
            pred_tail_sim = torch.cosine_similarity(sup_neg_e2[:, :self.num_query, :, :], e2, dim=-1)


            # print("e1",torch.max(cosloss).item(), torch.min(cosloss).item(), torch.mean(cosloss).item())
            # print("e2",torch.max(cosloss1).item(), torch.min(cosloss1).item(), torch.mean(cosloss1).item())
            # print("e3",torch.max(cosloss2).item(), torch.min(cosloss2).item(), torch.mean(cosloss2).item())

            target = torch.ones_like(pred_tail_sim)

            loss1 = self.metaR.margin_loss(pred_tail_sim, pred_head_sim, target)
            # print(loss,loss1)
            loss = loss + 0*loss1
            ...
            p_score, n_score, _, _, _, _ = self.metaR(task, iseval, curr_rel)
            y = torch.ones_like(p_score).to(self.device)
            loss = self.metaR.loss_func(p_score, n_score, y)
    ...
    def train(self):
        ...
        for e in range(self.epoch):
            ...
            if e % self.eval_epoch == 0 and e != 0:
                ...
                # valid_data1 = self.eval(istest=False, epoch=e)
                valid_data = self.eval(istest=True, epoch=e)
                # self.write_validating_log(valid_data, e)


                metric = self.parameter['metric']
                # best = valid_data1[metric]
                # print('\tBest model | {0} of valid set is {1:.3f}'.format(metric, best))
    ...
    def eval(self, istest=False, epoch=None):
        ...
        while True:
            ...
            # p_score [1,1] n_score [1, n_entity-1]
            # print("loss:",_)
